...
func (dbc *DB) MigrateToPartitionedTableWithConfig(cfg MigrationConfig) error {
	...
	// Step 1: Verify the target partitioned table exists
	l.Info("step 1: verifying target partitioned table exists")
	strategy, err := dbc.GetPartitionStrategy(targetTable)
	if err != nil {
		return fmt.Errorf("failed to check if target table exists: %w", err)
	}
	if strategy == "" {
		return fmt.Errorf("target table %s does not exist or is not partitioned - create it first using DDL or GORM AutoMigrate", targetTable)
	}
	l.WithField("strategy", strategy).Info("target table exists and is partitioned")
